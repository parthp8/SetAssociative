...
void accessData (
    ...
    int set_index = (addr >> (b)) & 0b11;

    // Cache hit
    for (int i = 0; i < E; i++) {
        if (cache[set_index][i].valid && cache[set_index][i].tag == tag) {
            (*hit_count)++;
            cache[set_index][i].recent_use = program_counter;
            return;
        }
    }
    ...
    if (!cache[set_index][0].valid || !cache[set_index][1].valid || !cache[set_index][2].valid || !cache[set_index][3].valid) {
        for (int i = 0; i < E && !cache[set_index][i].valid ; i++) {
            min_index = i;
        }
        cache[set_index][min_index].valid = 1;
        cache[set_index][min_index].tag = tag;
        cache[set_index][min_index].recent_use = program_counter;
        return;
    }

    size_t min = cache[set_index][0].recent_use;
    for ( int i = 1; i < E; i++) {
        if (cache[set_index][i].recent_use < min) {
            min_index = i;
            min = cache[set_index][i].recent_use;
        }
    }
